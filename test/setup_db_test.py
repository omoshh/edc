...
class TestSetupDB(unittest.TestCase):
    ...
    @patch(
        "backend.db.setup_db.time.sleep"
    )  # Мокаем sleep, чтобы тест не шел 20 секунд!
    @patch("backend.db.setup_db.get_conn")
    @patch("backend.db.setup_db.get_name")
    ...
    @patch("backend.db.setup_db.get_conn")
    @patch("backend.db.setup_db.get_name")
    def test_init_db_fails_after_10_attempts(
        self, mock_get_name, mock_get_conn, mock_sleep
    ):
        """Check behavior after 10 unsuccesful attemps"""
        mock_get_name.return_value = "metrics_table"
        mock_get_conn.return_value.__enter__.side_effect = psycopg.OperationalError(
            "Dead"
        )
