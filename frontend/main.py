...
with st.sidebar:
    ...
    selected_label = st.selectbox(
        "Select update frequency", options=list(REFRESH_OPTIONS.keys()), index=0
    )
...
def metrics():
    ...
    if df is not None and not df.empty:
        try:
            ...
            col3.metric(
                label="Load Average", value=f"{latest.get('load_average', 'N/A')}"
            )
            col4.metric(
                label="Network", value=f"{latest.get('network_bandwidth', 0)} Mb/s"
            )
...
def get_metrics_in_range(start, end):
    par = {"start": str(start), "end": str(end)}
    try:
    ...
    with col:
        if st.button(label, use_container_width=True):
            ...
            with st.spinner(f"Fetching last {label}..."):
                st.session_state.last_x_metrics = get_metrics_in_range(
                    start_time, end_time
                )
            st.rerun()
if (
    st.session_state.last_x_metrics is not None
    and not st.session_state.last_x_metrics.empty
):
    ...
    selected_labels = st.multiselect(
        "Metrics", options=options, default=options, key="custom_range"
    )
...
with left_col:
    ...
    start = st.datetime_input(
        "Choose start", value=today_midnight, format="DD.MM.YYYY", max_value="now"
    )
with right_col:
    end = st.datetime_input(
        "Choose end", value="now", format="DD.MM.YYYY", max_value="now"
    )
...
if st.session_state.df_metrics is not None and not st.session_state.df_metrics.empty:
    ...
    options = list(METRICS.keys())
    selected_labels = st.multiselect(
        "Metrics", options=options, default=options, key="ms_quick_intervals"
    )
    selected_columns = [METRICS[label] for label in selected_labels]
